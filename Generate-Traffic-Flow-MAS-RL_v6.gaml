...
//NPC
  species bus skills: [advanced_driving] { 
	...
	reflex time_to_restart when:(distance_left)<=1 or first_time = 1 or time_pass > 50{  //a_flag_checked_by_light = 0 and checked = false
         ...
         int f_p <- int(rnd(12));
         loop while: random_node = f_p or f_p = 5{f_p <- int(rnd(12));random_node <- int(rnd(12));}
...
//RL-agent
  species bus_agent skills: [advanced_driving,RSkill] { 
      ...
      list<point>route; 
      list state_1;list state_2;list state_3;
	   ...
	   action reward_calculate{
    	if(time_pass != 0){
    	    float punishment <- collision*0.1 + jam*0.0006 ;
    	    if(distance_front_car_before < 30 or time_after_waite>0){punishment <- collision*0.1 ;}//+ jam*0.0001
		    if(Instantaneous_speed<=target_speed and Instantaneous_speed != 0){ 
		        if(distance_front_car_before < 30 or time_after_waite>0){  //前面有车  或前面车离开不到3回合   或 出生or current_target = targets at 0 
		        	reward <- 0.001*(Instantaneous_speed/target_speed);
		        }  
			    else{reward <- 0.001 - 0.005*((target_speed-Instantaneous_speed)/target_speed);}////+ (time_pass > time_target ? -0.00013 : 0.00005); 
		    }
		    else if(Instantaneous_speed = 0){
		    	if(distance_front_car_before < 30 or time_after_waite>0 ){  //前面有车 
		        	reward <- 0;
		        }  
			    else{reward <- -0.001 ;}
			}
		    if(Instantaneous_speed>=target_speed){ //超速
		        //if(distance_behind_car_before < 20){reward <-0.001 - 0.03*((real_speed-target_speed)/target_speed);} //后面有车
			    //else{reward <- 0.001 - 0.0008*(real_speed/target_speed);}  ////+ (time_pass > time_target ? -0.0001 : 0.00005); //36 sigmiod 
			    reward <- 0.001 - 0.008*((Instantaneous_speed-target_speed)/target_speed);
		    }
    ...
    action queue_set{
    	if time_pass = 0{
    		cars_closest_front_1<-cars_closest_front_3;cars_closest_front_2<-cars_closest_front_3;
    		cars_closest_behind_1<-cars_closest_behind_3;cars_closest_behind_2<-cars_closest_behind_3;
            cars_closest_10_1<-cars_closest_10_3;cars_closest_10_2<-cars_closest_10_3;
    		state_1<-state_3;state_2<-state_3;
    		location_1<-location;location_2<-location;
    	}
    }
    action queue_change{
    	    cars_closest_front_1<-cars_closest_front_2; cars_closest_front_2<-cars_closest_front_3;
    		cars_closest_behind_1<-cars_closest_behind_2; cars_closest_behind_2<-cars_closest_behind_3;
            cars_closest_10_1<-cars_closest_10_2; cars_closest_10_2<-cars_closest_10_3;
    		state_1<-state_2; state_2<-state_3;
    		location_1<-location_2; location_2<-location;
    }
    	...
    	if(time_pass != 0){
		    ...
		    if(Instantaneous_speed <=target_speed and Instantaneous_speed != 0){ 
		        if(distance_front_car_before <= safe_interval or time_after_waite>0){  //前面有车  或前面车离开不到3回合    或 出生 ?or current_target = targets at 0 
			     ...
			     else{reward <- 0.001 - 0.002*((target_speed-Instantaneous_speed)/target_speed);}////+ (time_pass > time_target ? -0.00013 : 0.00005); 0.00066   0.0007*(target_speed/Instantaneous_speed)
		     }                                  //0.004
		     //速度0
		    else if(real_speed = 0 or Instantaneous_speed=0){ 
		    	  if(distance_front_car_before <= safe_interval or time_after_waite>0){  //前面有车 
		        	  reward <-  0.00001;//-0.00001; //0
		          }  
			     else{reward <- -0.0032 ;}
			}
			//超速
			    ...
			    reward <-0.001 - 0.005*((Instantaneous_speed-target_speed)/target_speed); //0.006
		    }                          //0.0045
