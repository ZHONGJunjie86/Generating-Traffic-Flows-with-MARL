/***
* Name: GenerateTrafficFlowMASRL
* Author: cdl
* Description: 
* Tags: Tag1, Tag2, TagN
***/

model GenerateTrafficFlowMASRL
global {   
   ...
   int train_agent <- 1;
	...
	file icon_bus_agent_MAS  <- file('../includes/icons/vehicles/normal_yellow.png');
	...
	init {  
	   ...
	   create bus_agent_train number: train_agent{   //Agent
			...
			min_safety_distance <- 0.01; // set up
			min_security_distance <- 0.01;
...
//NPC
  species bus skills: [advanced_driving] { 
	...
	reflex time_to_restart when:(distance_left)<=1 or first_time = 1 or time_pass > 50{  //a_flag_checked_by_light = 0 and checked = false
	     if(first_time != 1){
             done <- 1;
             do distance_cal;
         ...
         loop while: random_node = f_p or f_p = 4 or f_p = 1 or f_p = 9{f_p <- int(rnd(12));random_node <- int(rnd(12));}
...
//RL-agent
  species bus_agent skills: [advanced_driving,RSkill] { 
	  ...
	  float average_speed <- 16.0;
	  point pre_point;point initial_point; point location_0;point location_1;point location_2;
	  file Rcode_pause<-text_file("/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_R/R_pause.txt"); //file Rcode_clear<-text_file("D:/Software/GamaWorkspace/Python/R_clear.txt");
      file Rcode_read<-text_file("/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_R/R_read.txt");
      //list<bus>cars_front_behind;
      list<point>cars_closest_front_0;list<point>cars_closest_front_1;list<point>cars_closest_front_2;list<point>cars_closest_front_3;
      list<point>cars_closest_behind_0;list<point>cars_closest_behind_1;list<point>cars_closest_behind_2;list<point>cars_closest_behind_3;
      list<point>cars_closest_10_0;list<point>cars_closest_10_1;list<point>cars_closest_10_2;list<point>cars_closest_10_3;
      list<point>route; 
      list state_0;list state_1;list state_2;list state_3;
	   ...
	   action road_weight{
		if(m=0){
		 ...
		 if(distance_left>100){
		 	...
		 	time_target <- int((distance_left/100)*8)+ 2;//rnd(2) + 7; //4+9
		 }
    	...
    	if(time_pass != 0){
		    ...
		    if(Instantaneous_speed <=target_speed and Instantaneous_speed != 0){ 
		        if(distance_front_car_before <= safe_interval or time_after_waite>0){  //前面有车  或前面车离开不到3回合    或 出生 ?or current_target = targets at 0 
		        	reward <- 0;//0.00005*(Instantaneous_speed/target_speed);// 
		          }            //0.001
			     else{reward <- 0.001 - 0.003*((target_speed-Instantaneous_speed)/target_speed);}////+ (time_pass > time_target ? -0.00013 : 0.00005); 0.00066   0.0007*(target_speed/Instantaneous_speed)
		     }                                  //0.002
		     //速度0
		    else if(real_speed = 0 or Instantaneous_speed=0){ 
		    	  if(distance_front_car_before <= safe_interval or time_after_waite>0){  //前面有车 
		        	  reward <- 0;//0.0005//0.00001;
		          }  
			     else{reward <- -0.02 ;}//-0.002 //-0.0032
			}
			//超速
    ...
    action queue_set{
    	if time_pass = 0{
    		cars_closest_front_0<-cars_closest_front_3;cars_closest_front_1<-cars_closest_front_3;cars_closest_front_2<-cars_closest_front_3;
    		cars_closest_behind_0<-cars_closest_behind_3;cars_closest_behind_1<-cars_closest_behind_3;cars_closest_behind_2<-cars_closest_behind_3;
         cars_closest_10_0<-cars_closest_10_3;cars_closest_10_1<-cars_closest_10_3;cars_closest_10_2<-cars_closest_10_3;
            
    		state_0<-state_3;state_1<-state_3;state_2<-state_3;
    		
    		location_0<-location;location_1<-location;location_2<-location;
    	}
    }
    action queue_change{
    	   cars_closest_front_0<-cars_closest_front_1;cars_closest_front_1<-cars_closest_front_2; cars_closest_front_2<-cars_closest_front_3;
    		cars_closest_behind_0<-cars_closest_behind_1;cars_closest_behind_1<-cars_closest_behind_2; cars_closest_behind_2<-cars_closest_behind_3;
         cars_closest_10_0<-cars_closest_10_1;cars_closest_10_1<-cars_closest_10_2; cars_closest_10_2<-cars_closest_10_3;
    		state_0<-state_1;state_1<-state_2; state_2<-state_3;
    		location_0<-location_1;location_1<-location_2; location_2<-location;
    }
    
    action Python { //D:/Software/PythonWork\GAMA_python\Generate_Traffic_Flow_MAS_RL
    	 //do reward_calculate;
    	 do distance_cal;
         elapsed_time_ratio <- time_pass/time_target;
         elapsed_dis_ratio<- distance_passed/(distance_left+distance_passed); 
         on_linked_road_int <- on_linked_road = true?1:0;
         over <- (episode < total_episode ? 0 : 1);
         state_3 <- [real_speed/10, target_speed/10, elapsed_time_ratio, elapsed_dis_ratio,distance_front_car/10,distance_behind_car/10,0,done,over];//reward
    	 do queue_set;
    	 //
    	 save [initial_point,route] to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/route.csv" type: "csv" header: false;
    	 //
    	 save cars_closest_front_0 to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/save_NPC_front_0.csv" type: "csv" header: false;
    	 save cars_closest_front_1 to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/save_NPC_front_1.csv" type: "csv" header: false;
    	 save cars_closest_front_2 to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/save_NPC_front_2.csv" type: "csv" header: false;
    	 save cars_closest_front_3 to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/save_NPC_front_3.csv" type: "csv" header: false;
    	 //
    	 save cars_closest_behind_0 to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/save_NPC_behind_0.csv" type: "csv" header: false;
    	 save cars_closest_behind_1 to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/save_NPC_behind_1.csv" type: "csv" header: false;
    	 save cars_closest_behind_2 to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/save_NPC_behind_2.csv" type: "csv" header: false;
    	 save cars_closest_behind_3 to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/save_NPC_behind_3.csv" type: "csv" header: false;
    	 //
    	 save cars_closest_10_0 to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/save_NPC_0.csv" type: "csv" header: false;
    	 save cars_closest_10_1 to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/save_NPC_1.csv" type: "csv" header: false;
    	 save cars_closest_10_2 to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/save_NPC_2.csv" type: "csv" header: false;
    	 save cars_closest_10_3 to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/save_NPC_3.csv" type: "csv" header: false;
    	 //
    	 save location_0 to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/save_self_0.csv" type: "csv" header: false;
    	 save location_1 to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/save_self_1.csv" type: "csv" header: false;
    	 save location_2 to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/save_self_2.csv" type: "csv" header: false;
    	 save location to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/save_self_3.csv" type: "csv" header: false;
    	 //
    	 //write "state_0"+state_0;
    	 save state_0 to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_R/GAMA_intersection_data_0.csv"  type: "csv" header: false; 
    	 save state_0 to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_R/GAMA_intersection_data_0_0.csv"  type: "csv" header: false; 
    	 save state_1 to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_R/GAMA_intersection_data_1.csv"  type: "csv" header: false; 
    	 save state_2 to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_R/GAMA_intersection_data_2.csv"  type: "csv" header: false; 
    	 save state_3 to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_R/GAMA_intersection_data_3.csv"  type: "csv" header: false; 
       save [1,1] to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_R/D_A_T.csv"  type: "csv" header: false; 
         //write "read";
	     ...
	     loop while: check_receive = 0 {
	      ...
	      save [0] to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_R/python_AC_1.csv"  type: "csv" header: false; 
	      save [0] to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_R/python_AC_2.csv"  type: "csv" header: false;
	      jam<-0;
	      do queue_change;
      }    
    action collision_detection_action{
       ...
       cars_closest_front_3 <- [];cars_closest_behind_3 <- [];cars_closest_10_3<-[];
  	    list<bus_agent> buses_1 <- agents_inside(bus_agent); 
       list cars_front_behind_1 <-  buses_1 closest_to(self, 8);
       list<bus> buses_2 <- agents_inside(bus); 
       list cars_front_behind_2 <-  buses_2 closest_to(self, 8);
       list<bus_agent_train> buses_3 <- agents_inside(bus_agent_train); 
       list cars_front_behind_3 <-  buses_3 closest_to(self, 1);

        //先算出应有距离
	    loop bus_decided over: cars_front_behind_1 {
	        ...
	        if(empty(self.targets) = false and empty(bus_decided.targets) = false){
                ...
                if(current_target = bus_decided.current_target and current_road = bus_decided.current_road){ //=bus_decided.right_side_driving//and right_side_driving
            	    ...
            	    if((self.location distance_to current_target) > (bus_decided.location distance_to bus_decided.current_target)){
            	    	...
            	    	cars_closest_front_3<-cars_closest_front_3+bus_decided.location;
                    }
                    //agent 在前面，判断堵车
                    else{
            	    	...
            	    	cars_closest_behind_3<-cars_closest_behind_3+bus_decided.location;
                    }
                 }
                //不同路，同一个目标,下一个目标也一样
                if(current_road != bus_decided.current_road and current_target = bus_decided.current_target ){
                    ...
                    if(remain_self>1 and remain_bus_decided>1){
                	    ...
                	    if(next_2_point_bus_closest = next_2_point_self){
                	        cars_closest_10_3<-cars_closest_10_3+bus_decided.location;    
                	    }    	    
                	}
                }
                //不同路，不同一个目标
	    ...
	    //先算出应有距离
	    loop bus_decided over: cars_front_behind_2 {
	        float Euclidean_distance <- location distance_to bus_decided;
	        if(empty(self.targets) = false and empty(bus_decided.targets) = false){
                //同一条路上,同向,右行
                if(current_target = bus_decided.current_target and current_road = bus_decided.current_road){ //=bus_decided.right_side_driving//and right_side_driving
	    ...
	    //先算出应有距离
	    loop bus_decided over: cars_front_behind_3 {
	        float Euclidean_distance <- location distance_to bus_decided;
	        if(empty(self.targets) = false and empty(bus_decided.targets) = false){
                //同一条路上,同向,右行
                if(current_target = bus_decided.current_target and current_road = bus_decided.current_road){ //=bus_decided.right_side_driving//and right_side_driving
	    ...
	    if(Instantaneous_speed + acceleration <=0){
	    	...
	    	real_real_speed <- Instantaneous_speed*t + 0.5*acceleration*t*t;
	    ...
	    loop bus_decided over: cars_front_behind_1{
	        float Euclidean_distance <- location distance_to bus_decided;
	        if(empty(self.targets) = false and empty(bus_decided.targets) = false and time_pass!=0){
                //同一条路上,同向,右行
                if(current_target = bus_decided.current_target and current_road = bus_decided.current_road){ //=bus_decided.right_side_driving
                    ...
                    //agent 在前面，判断堵车
                    	...
                    	if((Euclidean_distance+real_real_speed) <= NPC_speed){jam<-1;}//write "同一条路上,同向,右行,Jam! "+Euclidean_distance+" "+distance_behind_car;}
                    }
                 }
                 //不同路，同一个目标,下一个目标也一样
                 if(current_road != bus_decided.current_road and current_target = bus_decided.current_target ){
                ...
                if(current_road != bus_decided.current_road and current_target != bus_decided.current_target ){ //and pass_target index_of next_point_self != 0
                	...
                	if(remain_self = 1 and remain_bus_decided > 1 and current_target != targets at 0){
                	    ...
                	    int self_target_before_index <- targets index_of current_target - 1; 
                        if(current_target = next_2_point_bus_closest and bus_decided.current_target = targets at self_target_before_index){
                    	    if((Euclidean_distance+real_real_speed) <= NPC_speed){jam<-1;}//write "不同路，同向,右行，Jam! "+Euclidean_distance+" "+distance_behind_car;}
                        }
                	}
                	if(remain_self > 1 and remain_bus_decided = 1 and bus_decided.current_target != bus_decided.targets at 0){
                		// agent 在后面，判断追尾
                	...
                	if(remain_self > 1 and remain_bus_decided > 1 ){
                        ...
                        if(current_target = next_2_point_bus_closest and current_target != targets at 0){
                        	int self_target_before_index <- targets index_of current_target - 1;
                    	    if((Euclidean_distance+real_real_speed) <= NPC_speed and bus_decided.current_target = targets at self_target_before_index){
	    ...
	    if(distance_front_car<30){time_target<-time_target+1;time_after_waite<-3;}  // or distance_behind_car<20
	...
	reflex change when :current_path = nil{		
        ...
        int random_node_start <- int(rnd(12)); 
        int random_node_end <- int(rnd(12));
		...
		location <- any_location_in(node_agt(random_node_start));
		final_target<- any_location_in(node_agt(random_node_end));
	}
	//信号に引っかかった後の処理  等红灯。。。
	reflex time_to_go when: a_flag_checked_pass_light != 0 and green_light_checked = true {
		if(true_target != nil){
	...
	//目的地（終着バスターミナル）についた時の処理  && 最初の時   location distance_to final_target
	reflex time_to_restart when:((distance_left)<=1 or first_time = 1 or collision = 1) and Cycle_LOCK = 0 { // //a_flag_checked_by_light = 0 and checked = false
	     if(first_time = 0){
             done <- 1;
          	 //do Python;
		       episode <- episode + 1 ; //加到1000，算完最后结果
             //write "done!!___________________________________";
             average_speed <-accumulated_speed/time_pass;
         }
         else{
         	first_time <- 0;
         	if start_R = 0 {do startR;start_R<-1;}  //startR only once
         }
         real_speed <- 0.0;time_pass <- 0;done<-0;
         int random_node_start <- int(rnd(12)); int random_node_end <- int(rnd(12));
         loop while: random_node_end = random_node_start or random_node_start = 4 or random_node_start = 1 or random_node_start = 9
         {random_node_end <- int(rnd(12));random_node_start <- int(rnd(12));}
         ...
         target<- node_agt(random_node_end);true_target <- node_agt(random_node_end);final_target <- node_agt(random_node_end).location;
		   ...
		   a_flag_checked_pass_light <- 1;
		   distance_passed <- 0.0;distance_all<- 0.0;reward<-0.0;collision<-0;jam<-0;Instantaneous_speed<-0.0;real_real_speed<-0.0;time_after_waite<-4;
		   accumulated_speed <-0.0;
		   do road_weight;
         target_speed<- min(distance_left/time_target,16);
         ...
         do collision_detection_action; //do Python;
	} 
	
	reflex move when: (current_path != nil or a_flag_checked_pass_light != 0 )and Cycle_LOCK = 0{// 道が決まり、目的地が決まれば動く  
	   accumulated_speed <-accumulated_speed + real_speed;
      //if(time_pass!=0){average_speed <-accumulated_speed/time_pass;}
	    if (episode<total_episode and done = 0 ) {
           real_speed <- real_real_speed;//real_speed + acceleration ;  //1;//
        }
		do drive;
	    time_pass <- time_pass + 1;
	...
	aspect icon {
		point loc <- calcul_loc();
			if(m =0){
			draw icon_bus_agent_MAS size: vehicle_length   at: loc rotate: heading + 90 ;
			}
			if(m = 1)
				{
			draw icon_bus_agent_MAS size: vehicle_length   at: loc rotate: heading + 90 ;	
...
//RL-train-agent
  species bus_agent_train skills: [advanced_driving,RSkill] { 
	  rgb color <- rgb(255,0,0); //rnd_
	  bool green_light_checked <-false;
  	  node_agt target; 
	  node_agt true_target;
   ...
   action Python {
   	 save [initial_point,route] to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/train/route.csv" type: "csv" header: false;
    	 save cars_closest_front to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/train/save_NPC_front.csv" type: "csv" header: false;
    	 save cars_closest_behind to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/train/save_NPC_behind.csv" type: "csv" header: false;
    	 save cars_closest_10 to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/train/save_NPC_closest_10.csv" type: "csv" header: false;
    	 save location to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_img/train/save_self.csv" type: "csv" header: false;
    	 do reward_calculate;
    	 do distance_cal;
       elapsed_time_ratio <- time_pass/time_target;
       elapsed_dis_ratio <- distance_left/100;//distance_passed/distance_all;///(distance_passed+distance_left); //    
       if(done =1){
       	write "elapsed_time_ratio "+elapsed_time_ratio+"time_target"+time_target;
       	list<bus_agent> bus_agents <- agents_inside(bus_agent);
       	list<float>NPC_speeds<-[];
       	float accumulated_speed<-0.0;int bus_number<-0; 
       	loop bus_decided over: bus_agents{
       		if bus_decided != nil{
       	     NPC_speeds<-NPC_speeds+bus_decided.average_speed;
       	     accumulated_speed<-accumulated_speed+bus_decided.average_speed;
       	     bus_number<-bus_number+1;
       	     }
         	}
         	//NPC_average_speed<-max(NPC_speeds);
       	    NPC_average_speed<-accumulated_speed/bus_number;
         }
       on_linked_road_int <- on_linked_road = true?1:0;
       over <- (episode < total_episode ? 0 : 1);
    	 save [Instantaneous_speed/10, target_speed/10, elapsed_time_ratio, elapsed_dis_ratio,distance_front_car/10,distance_behind_car/10,reward,done,over,NPC_average_speed] //time_pass
    	                                to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_R/GAMA_intersection_data_1.csv"  type: "csv" header: false; 
    	 save [Instantaneous_speed/10, target_speed/10, elapsed_time_ratio, elapsed_dis_ratio,distance_front_car/10,distance_behind_car/10,reward,done,over,NPC_average_speed] 
    	                                to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_R/GAMA_intersection_data_2.csv"  type: "csv" header: false; 
    	 save [Instantaneous_speed/10, target_speed/10, elapsed_time_ratio, elapsed_dis_ratio,distance_front_car/10,distance_behind_car/10,reward,done,over,NPC_average_speed] 
    	                                to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_R/GAMA_intersection_data_3.csv"  type: "csv" header: false; 
        
       save [0,0] to: "/home/cdl/gama_workspace/GAMA_python/Generate_Traffic_Flow_MAS_RL/GAMA_R/D_A_T.csv"  type: "csv" header: false; 
         //write "read";
	     loop s over:Rcode_read.contents{read_python<- R_eval(s);}
	     check_receive <- read_python at 0;
	     //waiting for python
	...
	action collision_detection_action{
       ...
       cars_closest_front <- [];cars_closest_behind <- [];cars_closest_10<-[];
  	    //list<bus> buses <- agents_inside(bus_agent);//agents_inside(bus)+
        //cars_front_behind <-  buses closest_to(self, 10) ;
        
       list<bus_agent> buses_1 <- agents_inside(bus_agent); 
       list cars_front_behind_1 <-  buses_1 closest_to(self, 10);
       list<bus> buses_2 <- agents_inside(bus); 
       list cars_front_behind_2 <-  buses_2 closest_to(self, 10);
       ...
       if(Instantaneous_speed + acceleration<=0 ){ 
	    ...
	    loop bus_decided over: cars_front_behind_1{
	        float Euclidean_distance <- location distance_to bus_decided;
	        if(empty(self.targets) = false and empty(bus_decided.targets) = false and time_pass >3){
                //同一条路上,同向,右行
                if(current_target = bus_decided.current_target and current_road = bus_decided.current_road){ //=bus_decided.right_side_driving
                    // agent 在后面，判断追尾 //s=v0t + 1/2at^2
            	    if((self.location distance_to current_target) >= (bus_decided.location distance_to bus_decided.current_target)){
   ...
   reflex collision_detection when :first_time =0 {   
   	     do collision_detection_action;
   	     /*/ write "collision_detection";
   	    if Cycle_LOCK =0 {
            do collision_detection_action;  
            if collision = 1{Cycle_LOCK<-1;}
           }
	...
	reflex change when :current_path = nil{		
      ...
      int random_node_end <- int(rnd(12));
      loop while: random_node_end = random_node_start or random_node_start = 4 or random_node_start = 1 or random_node_start = 9
               {random_node_end <- int(rnd(12));random_node_start <- int(rnd(12)); }//random_node_start <- 5;}//
	...
	//目的地（終着バスターミナル）についた時の処理  && 最初の時   location distance_to final_target
	reflex time_to_restart when:((distance_left) = 0 or first_time = 1 or collision = 1 or time_pass>=120){  //and Cycle_LOCK=0   a_flag_checked_by_light = 0 and checked = false
	     if(first_time = 0){
             done <- 1;
          	 do Python;
          	 //Cycle_LOCK<-1;
         ...
         route<-[];loop i over: targets{route<-route+any_location_in(node_agt(i));}
         initial_point <- location;
         do collision_detection_action; //if Cycle_LOCK = 0{write " time_to_restart"; }//do Python;
	} 
...
experiment traffic_simulation type: gui {
	...
	output {
		display city_display type: opengl{
			...
			species bus_agent_train aspect: icon;
		}
	}
}
