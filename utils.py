...
def GAMA_connect(test):
    ...
    while error == True:
        try:
            ...
            time_pass = state_1[2];time_pass = state_2[2];time_pass = state_3[2]
            error = False
        except (IndexError,FileNotFoundError,ValueError,OSError):
            time.sleep(0.003)
            error = True
        
    state_1 = np.delete(state_1, [6,7,8], axis = 0) #4,5,
    state_2 = np.delete(state_2, [6,7,8], axis = 0)
    state_3 = np.delete(state_3, [6,7,8], axis = 0)
